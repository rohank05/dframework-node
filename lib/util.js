...
export default {
     ...
     * @throws {Error} When collection is not an array or key is not a string
     * @example
    ...
    join: function ({ left, right, join, columns }) {
        if (!Array.isArray(left)) {
            throw new Error('Left array is required and must be an array');
        }
        if (!Array.isArray(right)) {
            throw new Error('Right array is required and must be an array');
        }
        if (!Array.isArray(join) || join.length !== 2) {
            throw new Error('Join must be an array with exactly 2 elements [leftKey, rightKey]');
        }
