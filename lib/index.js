...
class Framework {
    ...
    async setSql(sqlConfig) {
        let sql;
        if (sqlConfig) {
            if (typeof sqlConfig !== 'object') {
                throw new Error('SQL configuration must be an object');
            }
            if (!sqlConfig.server) {
                throw new Error('SQL server is required in configuration');
            }
            if (!sqlConfig.database) {
                throw new Error('SQL database is required in configuration');
            }
    ...
    async setMySql(mysqlConfig) {
        ...
        if(mysqlConfig) {
            if (typeof mysqlConfig !== 'object') {
                throw new Error('MySQL configuration must be an object');
            }
            if (!mysqlConfig.host) {
                throw new Error('MySQL host is required in configuration');
            }
            if (!mysqlConfig.database) {
                throw new Error('MySQL database is required in configuration');
            }
    ...
    async login(credentials) {
        if (!credentials || typeof credentials !== 'object') {
            throw new Error('Credentials object is required for login');
        }
     ...
     * @throws {Error} When controller name is invalid
     * @example
     ...
     * @throws {Error} When no controller names are provided or invalid names are given
     * @example
    ...
    createControllers() {
        if (arguments.length === 0) {
            throw new Error('At least one controller name must be provided');
        }
        
        for (const name of arguments) {
            if (!name || typeof name !== 'string') {
                throw new Error(`Controller name must be a non-empty string, got: ${typeof name}`);
            }
